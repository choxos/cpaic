...
#>   Provenance: cpaic 0.1.0, rstan 2.36.0.9000, Stan md5 0a55fe50, seed 1
...
#>   component estimate        se      lower     upper
#> 1       BUP       NA        NA         NA        NA
#> 2       CBT 0.407785 0.1383116 0.08066389 0.6410474
#> 3       NRT       NA        NA         NA        NA
#> 4       VAR       NA        NA         NA        NA
```
...
#>    CBT+NRT        NRT    1.518 0.138 1.084 1.898  0.988          exact
#>    CBT+VAR        NRT    2.831 0.201 1.799 4.032  0.999          exact
...
#>        CBT        NRT       NA    NA    NA    NA     NA not identified
#>    CBT+BUP        NRT       NA    NA    NA    NA     NA not identified
#>    CBT+NRT        NRT    1.815 0.144 1.287 2.310  0.999          exact
#>    CBT+VAR        NRT    2.236 0.240 1.331 3.426  0.994          exact
#>         UC        NRT       NA    NA    NA    NA     NA not identified
#>   NA = not uniquely estimable from this component design (see estimable_effects()).
#>   `se` is on the link (log) scale; the interval is back-transformed.
```
...
|:---------------|--------:|-----:|------:|--------:|:----------------|
| CBT+NRT vs NRT |    1.63 | 1.66 |  1.68 |    1.52 | (1.08, 1.90)    |
| CBT+VAR vs NRT |    3.21 | 3.23 |  3.25 |    2.83 | (1.80, 4.03)    |
...
|:---|:---|:---|:---|:---|:---|:---|
| CBT | CBT |  |  |  |  | 1.52 (1.08, 1.90) |
| CBT+BUP |  | CBT+BUP |  |  |  |  |
| CBT+NRT |  |  | CBT+NRT | 0.55 (0.40, 0.72) | 1.52 (1.08, 1.90) |  |
| CBT+VAR |  |  | 1.86 (1.39, 2.52) | CBT+VAR | 2.83 (1.80, 4.03) |  |
| NRT |  |  | 0.67 (0.53, 0.92) | 0.37 (0.25, 0.56) | NRT |  |
| UC | 0.67 (0.53, 0.92) |  |  |  |  | UC |
...
#>      CBT    0.408  0.988           1     1.012 0.988
#>       UC    0.000  0.013           2     1.988 0.012
...
erodes steadily, and `CBT`’s SUCRA falls from 0.998 in the
lightest-smoking population to 0.877 in the heaviest, which is what a
...
| CBT vs UC     |    1.63 | 1.66 | (1.43, 1.92) | yes    |  1.68 |    1.52 |
...
|:---------|------:|------:|------:|------:|------:|---------:|
| tau\[1\] | 0.125 | 0.127 | 0.008 | 0.386 | 1.007 |  528.038 |
...
#>  random       0.0     0.0      NA                 5 k_psis > 0.7
#>   fixed      -0.3     0.8    0.64 |elpd_diff| < 4 6 k_psis > 0.7
...
#>        CBT        NRT   -0.105  -0.062 -0.129        0.042       0.024        0.042     FALSE
#>    CBT+BUP        NRT    0.301   0.379  0.236        0.078       0.065        0.078     FALSE
#>    CBT+NRT        NRT    0.408   0.441  0.399        0.034       0.009        0.034      TRUE
#>    CBT+VAR        NRT    1.021   1.056  1.004        0.036       0.017        0.036      TRUE
#>         UC        NRT   -0.512  -0.504 -0.527        0.009       0.015        0.015     FALSE
```
