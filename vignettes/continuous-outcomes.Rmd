...
#> Chain 4 Exception: normal_lpdf: Scale parameter is 0, but must be positive! (in '/var/folders/2h/yqztgsf96gsbkqn60cymgsjr0000gn/T/Rtmpat5eRU/model-116ceea50c0.stan', line 122, column 19 to column 50)
...
* `Diet` and `SU` are identified only from **aggregate** data. The `Met`
  ...
  a slope, and that slope is a between-study gradient. This is ecological
  inference. It is not the same currency as a within-trial interaction, and cpaic
...
ggplot(curve, aes(8 + x, estimate, colour = treatment, fill = treatment)) +
...
<img src="figure/curve-1.png" alt="plot of chunk curve" width="100%" />
...
edges enter exactly as published, at **their own** populations. Look at what
that does to the `Met` component, which is informed only by the two aggregate
monotherapy trials:
...
`SGLT2` and `SU` are close to the truth: they ride on adjusted IPD contrasts.
`Met` and `DPP4` are not: they ride on unadjusted aggregate contrasts estimated
in populations with a different baseline HbA1c, and nothing in the two-stage
route can move them. The additive model even notices that something is wrong,
though it cannot say what:
...
That lack of fit is not evidence against additivity. It is the two aggregate
metformin trials disagreeing with each other, because they enrolled populations
half a percentage point apart in baseline HbA1c and metformin's effect depends
on baseline HbA1c. The one-stage model reconciles them by integrating the
individual-level model over each study's own covariate distribution. The
two-stage model has no mechanism to do so.
...
reason to report DIC alongside.
