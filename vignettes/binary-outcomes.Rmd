...
Look also at `cmaic()` against `cstc()`. For the large `CBT+VAR` effect the
marginal estimate is visibly pulled toward the null relative to the conditional
one; for the small `CBT+NRT` effect the two are indistinguishable. That is
non-collapsibility behaving exactly as advertised: the marginal-conditional gap
grows with the size of the effect and with the prognostic variance you are not
conditioning on, and vanishes as either goes to zero. Neither method is wrong;
they are answering different questions, and the gap between the answers is a
feature of the logit link.
