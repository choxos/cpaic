...
) jointly models individual and aggregate data. Post-fit reporting
includes average conditional effects on the link scale at supplied
covariate means and treatment-level marginal contrasts standardized over
an explicit target covariate distribution, with empirical or named
summary targets, supported continuous margins, and moment and
joint-dependence fidelity checks. Survival summaries begin at model time
zero and use donor-study follow-up support; nonlinear marginal effects
remain treatment-level summaries. Models are implemented for binary,
continuous, count, and time-to-event outcomes. This methodology and
