...
analyzed directly. Two ideas each solve half of the problem:

- **Component NMA** (Rücker et al. 2020) decomposes multi-component treatments
  into additive component effects. If sub-networks *share components*, those
  component effects bridge the gap; but the method uses aggregate data only and
  ignores effect-modifier imbalance.
...
These are different words that happen to be spelled the same, and conflating
them causes real confusion:

- **Anchored PAIC** (the NICE sense): the adjusted comparison runs through a
  *common comparator arm* within each trial, rather than comparing single arms.
  `cmaic()` and `cstc()` are anchored in this sense.
- **Anchored cNMA** (the Rücker/Wigle sense): an *inactive component* is fixed
  at zero in the component parameterization. Setting `inactive = NULL` gives the
  **unanchored** parameterization of Wigle & Béliveau (2022), in which every
  unit receives its own parameter and no anchor can be misspecified.

Bridging a disconnected network through shared components is **not** anchoring
by a common comparator. It is identification through additivity.
...
- **Bayesian `cmlnmr()`** (component-additive ML-NMR), fitted with rstan by
  ...
  Binary, continuous, count, and survival outcomes, with separate conditional
  and target-standardized marginal reporting paths.
...
Reconnecting a network through shared components does **not** guarantee that
the effects you want are estimable. A relative effect is uniquely estimable
exactly when its contrast vector lies in the **row space** of the component
design matrix `X = B C` (Wigle et al. 2026). Full column rank of `X` is
sufficient but *not necessary*, so a rank-deficient network can still identify
useful cross-sub-network contrasts.

This matters because both engines will otherwise hand back a confident-looking
number for a contrast that carries no information: the frequentist fit through
the Moore-Penrose pseudoinverse, and the Bayesian fit through the prior.
`cpaic` checks every contrast and returns `NA` instead.
...
link-scale effect at those means.
...
Marginal reporting uses a separate target-distribution interface. It averages
treatment-specific outcomes within every posterior draw, then forms the
requested contrast. For example, an empirical binary target distribution can
be supplied as actual 0/1 rows with weights:
...
The donor study supplies the fitted intercept needed for absolute risks. The
target rows remain a separate input. Gaussian mean differences and Poisson rate
ratios do not need a donor baseline because the common intercept cancels.

An empirical target is a data frame with exactly the fitted effect modifiers.
Its finite, nonnegative weights are normalized and zero-weight rows are
discarded. Bernoulli modifiers must be represented by actual 0/1 rows. A
summary target is a named list of `means`, `sds`, and `margins`, with optional
latent-scale `cor` and deterministic Sobol' `n_int`. Continuous modifiers can
use `normal`, `gamma`, `lognormal`, or `beta` margins independently; fitted
Bernoulli modifiers must remain Bernoulli. Independent Bernoulli strata are
enumerated exactly, while continuous margins use deterministic integration
nodes. Summary targets have moment-fidelity gates for Bernoulli strata and
non-normal means and SDs. The Bernoulli prevalence tolerance is
`max(1e-8, 0.1 * min(p, 1 - p))`; non-normal means must be within 0.1 target SD
and non-normal SDs within 15 percent relative error. A failed gate stops with a
request for more nodes or an empirical target. Mixed or non-normal
multivariable targets also require resolved pairwise correlations to agree
within 0.05 of a high-resolution deterministic reference grid.
Target-distribution uncertainty is not propagated, and these checks do not
constitute a formal overlap or positivity diagnostic.

For survival, predictions start at model time zero and are not landmark or
delayed-entry conditioned. `times` must lie within the observed follow-up
support of the selected donor study, which may be shorter than the global
support. Survival and risk ratios, piecewise-exponential RMST ratios, and
time-specific marginal hazard ratios are calculated on the log scale with
stable log-sum-exp operations before optional back-transformation. RMST
measures require a piecewise-exponential donor baseline and are evaluated
analytically over its intervals. M-spline fits support marginal survival, risk,
and time-specific marginal hazard measures, but not RMST.
Nonlinear marginal effects are treatment-level contrasts; the package does not
report marginal component effects or rankings.

`relative_effects()`, `marginal_effects()`, `league_table()`,
`component_effects()`, `estimable_effects()`, `additivity_test()`,
`effective_sample_size()`, `forest()` and `plot()` summarize and visualize a
fit.
...
- **Conditional and marginal cML-NMR summaries are separate estimands.** A
  one-row `newdata` profile reports an average conditional link-scale contrast.
  `marginal_effects()` requires a target distribution and standardizes absolute
  outcomes before forming treatment-level contrasts. Nonlinear marginal
  effects are not component-additive, so marginal component effects and
  marginal treatment rankings are not reported.
- **Some marginal measures borrow a study baseline.** Binomial effects,
  Poisson rate differences, and every survival measure transport a selected
  fitted study intercept or baseline hazard to the target population. This is
  an explicit assumption. Target-distribution uncertainty is not propagated,
  and study-arm random effects are set to zero for population-mean predictions.
  Survival predictions begin at model time zero, not at a landmark or delayed
  entry time, and prediction times are restricted to the selected donor study's
  observed follow-up support.
- **Target checks are not overlap diagnostics.** Empirical and summary targets
  are checked for finite values, fitted-margin support, Bernoulli strata, and
  summary moment fidelity. The package does not estimate a formal overlap or
  positivity statistic, so extrapolation risk remains an analyst-level
  responsibility.
- **Survival conditions on the supplied rows.** `cmlnmr()` evaluates the exact
  ...
  assumed. `marginal_effects()` can report standardized survival and risk
  contrasts and time-specific marginal hazard ratios within the fitted follow-up
  support. Restricted mean survival time contrasts are available for
  piecewise-exponential baselines. M-spline fits retain the pointwise survival,
  risk, and time-specific hazard measures. These outputs additionally require
...
- **Survival interfaces have different status contracts.** The two-stage
  ...
  contrasts through `relative_effects()`. A scalar marginal hazard ratio is not
  reported because the standardized hazard ratio generally varies with time.
...
- Wigle A, Béliveau A (2022). Bayesian unanchored additive models for component
  network meta-analysis. *Statistics in Medicine* 41(22):4444-4466.
- Wigle A, Béliveau A, Nikolakopoulou A, Lin L (2026). Creating treatment and
  component hierarchies in component network meta-analysis.
- Efthimiou O, et al. (2022). A Bayesian model for combining aggregate and
  individual participant data in component network meta-analysis. *Statistics
  in Medicine* 41(14):2586-2606.
