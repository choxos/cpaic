...
A factorial simulation evaluates the **cross-gap** contrast on a
disconnected network, i.e. the comparison no trial makes. The simulation
code and outputs currently live in the local `documentation/` tree,
which is not part of the public repository, so the figures below are
indicative rather than independently reproducible; a tracked,
reproducible validation pipeline is planned.
